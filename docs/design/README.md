...
| [spec-shelf-titles-across-the-board.md](spec-shelf-titles-across-the-board.md) | **Gated and on `nightly` 2026-09-24; not yet deployed.** A library-wide shelf-title audit and its fixes. N1: a non-Latin Goodreads name shelves under its bracketed romanization (Mushoku Tensei). C1: the author gate accepts a work credited to ANY of a book's authors (Pendergast, METAtropolis). Banished Lands declared an umbrella. Q1: an edition listing never shelves a book on its own (Under the Dome). Q-author: the search queries a PERSON -- combined pen-name and role credits ("Andrews & Wilson", "- introduction") no longer become the query author (Tier One). Bundle folder rule for the folder-junk shelves (v1.3.216). Every rule A/B'd by record/replay with 0 regressions; one standing red (Sherlock, an O2-exposed title-gate defect, open). |
