...
| [spec-shelf-titles-across-the-board.md](spec-shelf-titles-across-the-board.md) | **N1/C1/Q1/Q-author LIVE on prod 2026-09-24 (`0c8b7e3`); S1 gated on `nightly`.** A library-wide shelf-title audit and its fixes. N1: a non-Latin Goodreads name shelves under its bracketed romanization (Mushoku Tensei). C1: the author gate accepts a work credited to ANY of a book's authors (Pendergast, METAtropolis). Banished Lands declared an umbrella. Q1: an edition listing never shelves a book on its own (Under the Dome). Q-author: the search queries a PERSON -- combined pen-name and role credits ("Andrews & Wilson", "- introduction") no longer become the query author (Tier One). S1: when the title+author search adopts nothing, the listing of a sibling by the same author is read and its numbered members tried under the strict title gate (King and Maxwell -> Sean King & Michelle Maxwell #6; library-wide 28 engaged, 0 movers). Bundle folder rule for the folder-junk shelves (v1.3.216); the folder fallback itself becomes opt-in, default off (v1.3.217, about 32 prod shelves drop on refresh, ~20 of them junk). Every rule A/B'd by record/replay with 0 regressions; one standing red (Sherlock, an O2-exposed title-gate defect, open). |
